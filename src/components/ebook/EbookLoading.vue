    ...
    this.task = setInterval(() => {
      this.$refs.mask.forEach((item, index) => {
        ...
        if (index === 0) {
          ...
          if (this.add && maskWidth.value < 16) {
            let preMaskWith = this.maskWidth[index - 1]
            if (preMaskWith.value > 8) {
              maskWidth.value++
              lineWidth.value-- 
            } 
          } else if (!this.add && lineWidth.value < 16) {
            let preLineWith = this.lineWidth[index - 1]
            if (preLineWith.value > 8) {
              maskWidth.value--
              lineWidth.value++
            } 
          }
